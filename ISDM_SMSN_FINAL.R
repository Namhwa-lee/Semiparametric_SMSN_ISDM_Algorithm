...
res <- snmix.isdm(data=data, grid=20, constant=0.1)
res$Iteration
res$Directional_Derivative
res$location_parameter ; res$skewness_parameter
all(diff(res$Observed_Loglik)>0)
...
points(sort(data), res$NPMLE[order(data)], col="blue", lwd=2, type="l", lty=2)
curve(0.6*dsn(x, xi=0, omega=1, alpha=5)+0.4*dsn(x, xi=0, omega=3, alpha=5), n=1000, add=TRUE, col='red', lwd=2)
legend("topright", legend = c("True Density", "NPMLE by ISDM"),
       col = c("red", "blue"), lty=c(1,1), lwd=2, cex=1.3)

res1 <- snmix.isdm(data=data1, grid=20, constant=0.1)
res1$Iteration
res1$Directional_Derivative
res1$location_parameter ; res1$skewness_parameter
all(diff(res1$Observed_Loglik)>0)

hist(data1, nclass=25, freq = FALSE, ylim=c(0,.5), main="Histogram : Scale Mixture of Skew Normal")
points(sort(data1), res1$NPMLE[order(data1)], col="blue", lwd=2, type="l", lty=2)
curve(0.6*dsn(x, xi=0, omega=1, alpha=5)+0.4*dsn(x, xi=0, omega=3, alpha=5), n=1000, add=TRUE, col='red', lwd=2)
legend("topright", legend = c("True Density", "NPMLE by ISDM"),
       col = c("red", "blue"), lty=c(1,1), lwd=2, cex=1.3)
